...
Original file is located at
    https://colab.research.google.com/drive/1pSjCpn73v6DWTG4AuClACGjIc0qmPz7S
"""
...
if __name__ == '__main__':
  ...
  (p | 'ReadData' >> beam.io.ReadFromText('gs://newsbot-wrhf/craft-beer/beers.csv', skip_header_lines=1)
     | 'SplitData' >> beam.Map(lambda x: x.split(','))
     | 'FormatToDict' >> beam.Map(lambda x:{"sr":x[0], "abv":x[1],"ibu":x[2],"id":x[3],"name":x[4],"style":x[5],"brewery_id":x[6], "ounces":x[7]})
     | 'DeleteIncompleteData' >> beam.Map(discard_incomplete)
     | 'ChangeDataType' >> beam.Map(convert_types)
     | 'DeleteUnwantedData' >> beam.Map(del_unwanted_cols)
     | 'WrtieToBigQuery' >> beam.io.WriteToBigQuery(
         '{0}:craft_beer.transformed_beer_data'.format(PROJECT_ID),
         schema=SCHEMA,
